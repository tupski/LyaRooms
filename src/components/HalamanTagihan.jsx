    ...
    <div className="min-h-screen p-4 pt-6 pb-28">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto space-y-5">
        ...
        <div className="grid grid-cols-4 gap-1 p-1 rounded-full bg-black/10">
          <button onClick={() => setActiveMenu('bulanan')} className={`py-2 px-2 rounded-full text-xs font-semibold transition-all duration-300 ${activeMenu === 'bulanan' ? 'bg-gradient-to-r from-pink-400 to-pink-600 text-white shadow-lg' : 'text-gray-700'}`}>
            Tgh. Unit
          </button>
          <button onClick={() => setActiveMenu('fee')} className={`py-2 px-2 rounded-full text-xs font-semibold transition-all duration-300 ${activeMenu === 'fee' ? 'bg-gradient-to-r from-pink-400 to-pink-600 text-white shadow-lg' : 'text-gray-700'}`}>
            Tgh. Fee
          </button>
          <button onClick={() => setActiveMenu('pengeluaran')} className={`py-2 px-2 rounded-full text-xs font-semibold transition-all duration-300 ${activeMenu === 'pengeluaran' ? 'bg-gradient-to-r from-pink-400 to-pink-600 text-white shadow-lg' : 'text-gray-700'}`}>
            Pengeluaran
          </button>
    ...
    <div className="space-y-5">
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        ...
        <DialogContent className="bg-white">
          ...
          <DialogFooter><Button onClick={handleAddTagihan} disabled={isSubmitting} className="w-full bg-pink-600 hover:bg-pink-700">Simpan Tagihan</Button></DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          !unpaidQuery.isLoading && tagihanList.map(tagihan => {
            ...
            return (
              <motion.div key={tagihan.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/50 p-4 rounded-2xl shadow-sm border relative">
                ...
                <p className="text-lg font-bold text-pink-600">{formatRupiah(tagihan.amount)}</p>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-4 space-y-3 overflow-hidden">
              ...
              {!paidQuery.isLoading && paidList.length > 0 ? paidList.map(item => (
                <motion.div key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/50 p-4 rounded-2xl relative">
                  ...
                  <p className="text-pink-700 font-semibold">{formatRupiah(item.amount)}</p>
      ...
      <div className="glassmorphic-card p-5 space-y-4">
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Coins className="text-pink-500 shrink-0" /> Tagihan Fee ({formatRupiah(totalUnpaidFee)})</h2>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 pb-2 border-b border-slate-100">
          <div className="relative group">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400 group-focus-within:text-pink-500 transition-colors" />
            <input
              type="text"
        ...
        {processedFees.length === 0 ? (
          ...
          processedFees.map((fee) => (
            <motion.div key={fee.nama} layout className="bg-white/50 border p-4 rounded-2xl relative">
              ...
              <p className="text-gray-700">Total Fee: <span className="font-bold text-xl text-pink-600">{formatRupiah(fee.totalFee)}</span></p>
            ...
            <div className="space-y-3">
              <div className="rounded-xl border bg-slate-50 p-3 text-sm">
                ...
                <div className="mt-1 flex items-center justify-between">
                  ...
                  <span className="font-bold text-pink-700">{formatRupiah(modalMarketing.totalFee)}</span>
                </div>
              </div>
                ...
                {modalMarketing.transactions.map((t) => (
                  <div key={t.transaction_id} className="flex items-center justify-between gap-3 rounded-xl border p-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs font-semibold text-pink-700">{formatRupiah(t.fee)}</p>
                    </div>
                    <Button
                      size="sm"
              ...
              <div className="space-y-2 border-t pt-3">
                ...
                <input type="file" accept="image/*" onChange={(e) => setUploadFile(e.target.files[0])} className="w-full text-sm text-gray-700 file:text-pink-600" />
                <Button
                  className="w-full bg-pink-600 hover:bg-pink-700"
                ...
                <>
                  {paidFees.map(item => (
                    <motion.div key={item.id} layout className="bg-white/50 border p-4 rounded-2xl relative">
                      ...
                      <p className="text-pink-700 font-semibold">{formatRupiah(item.total_fee)} ({item.customer_count} CS)</p>
                      <p className="text-xs text-gray-500">Lunas: {formatLunasDateTimeWib(item.paid_at)}</p>
        ...
        <div className="space-y-4 py-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={selectAllCategories} className="text-xs text-pink-600 hover:underline">Pilih Semua</button>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map(cat => (
                <button key={cat} type="button" onClick={() => handleCategoryToggle(cat)}
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold transition ${selectedCategories.includes(cat) ? 'bg-pink-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={selectAllLocations} className="text-xs text-pink-600 hover:underline">Pilih Semua</button>
            ...
            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
              {locations.map(loc => (
                <button key={loc} type="button" onClick={() => handleLocationToggle(loc)}
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold transition ${selectedLocations.includes(loc) ? 'bg-pink-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  {loc}
                </button>
              ))}
            </div>
...
const PengeluaranUnit = ({ onDataUpdate }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glassmorphic-card p-5 space-y-4">
        <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Building2 className="w-5 h-5 text-pink-500" /> Filter Unit</h2>
        <div className="grid grid-cols-2 gap-3">
          <div>
                ...
                <div className="px-4 pb-3 pt-1 space-y-2 border-t bg-white/30">
                  ...
                  {isCategoryLoading && (
                    <div className="flex justify-center py-3">
                      <Spinner className="w-5 h-5 text-pink-500" />
                    </div>
                  )}
