    ...
    return (
      <div className="flex items-center gap-2 rounded-2xl bg-red-50 border border-red-200 px-4 py-2.5 text-sm text-red-800 shadow-sm">
        ...
        <div className="min-w-0">
          <span className="font-semibold">Hari Ini Libur Nasional: </span>
          <span>{todayInfo.holidayName}</span>
          <span className="text-red-500 text-xs ml-1">— {todayInfo.dateLabel}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      {tomorrowInfo?.isHoliday && (
        <div className="flex items-center gap-2 rounded-2xl bg-amber-50 border border-amber-200 px-4 py-2.5 text-sm text-amber-800 shadow-sm">
          ...
          <div className="min-w-0">
            <span className="font-semibold">Besok Libur: </span>
            <span>{tomorrowInfo.holidayName}</span>
            <span className="text-amber-500 text-xs ml-1">— {tomorrowInfo.dateLabel}</span>
          </div>
        </div>
      )}
      {todayInfo.isWeekend && (
        <div className="flex items-center gap-2 rounded-2xl bg-green-50 border border-green-200 px-4 py-2.5 text-sm text-green-800 shadow-sm">
          <span className="text-base">🌿</span>
          <div className="min-w-0">
            <span className="font-semibold">Hari ini Weekend</span>
            <span className="text-green-600 text-xs ml-1">— {todayInfo.dayName}, {todayInfo.dateLabel}</span>
          </div>
        </div>
      )}
    </div>
